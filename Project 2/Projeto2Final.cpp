...
/*
void printGraph(vector <vector <pair<int,int> > > & aux, int V)
{
    for (int v = 0; v < n+2; ++v)
    {
        ...
        for (int i = Y; i != X; i = parents[i]) {
            ...
            //for (auto x : residual_adj[a]){
                if (residual_adj[a][x].first == i){
                    ...
                    //printf("first e segundo %d %d\n",residual_adj[a][x].first,residual_adj[a][x].second);
                    break;
                }
            }
